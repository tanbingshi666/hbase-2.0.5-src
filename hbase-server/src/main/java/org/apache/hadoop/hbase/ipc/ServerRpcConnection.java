...
abstract class ServerRpcConnection implements Closeable {
    ...
    public void processOneRpc(ByteBuff buf) throws IOException,
        ...
        if (connectionHeaderRead) {
            ...
            // 处理 RPC 连接头部 (也即该 RPC 请求找到了对应的服务)
    ...
    /**
     * @param buf Has the request header and the request param and optionally
    ...
    protected void processRequest(ByteBuff buf) throws IOException,
        ...
        if (!this.rpcServer.scheduler.dispatch(
                // 封装 NettyServerCall 为 CallRunner
                new CallRunner(this.rpcServer, call))
        ) {
