...
public class RpcServerFactory {
    ...
    public static RpcServer createRpcServer(final Server server, final String name,
         ...
         * hbase.pb.RegionServerStatusService,
         * hbase.pb.LockService,
         * hbase.pb.HbckService,
         * hbase.pb.ClientService,
         * hbase.pb.AdminService
         */
        LOG.info("Creating " + rpcServerClass + " hosting " + servicesList);

        // 反射创建 NettyRpcServer 并调用其构造方法
        return ReflectionUtils.instantiateWithCustomCtor(
                rpcServerClass,
                new Class[]{Server.class, String.class, List.class, InetSocketAddress.class, Configuration.class, RpcScheduler.class, boolean.class},
