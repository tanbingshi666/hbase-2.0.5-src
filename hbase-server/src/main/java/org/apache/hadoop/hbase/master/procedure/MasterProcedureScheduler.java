...
public class MasterProcedureScheduler extends AbstractProcedureScheduler {
    ...
    protected void enqueue(final Procedure proc, final boolean addFront) {
        // 判断是否 meta 表操作
        if (isMetaProcedure(proc) || (isTableProcedure(proc) && getTableName(proc).equals(TableName.META_TABLE_NAME))) {
            ...
            doAdd(tableRunQueue,
                    // 获取表队列 TableQueue
                    getTableQueue(getTableName(proc)),
                    proc, addFront);
    ...
    private <T extends Comparable<T>> void doAdd(FairQueue<T> fairq, Queue<T> queue,
        ...
        // 对于创建表而言 添加 CreateTableProcedure 到 TableQueue
        ...
        if (reason != null) {
            // 对于创建表而言 添加 TableQueue 到 tableRunQueue = FairQueue
    ...
    protected Procedure dequeue(boolean onlyUrgent) {
        ...
        // 1 优先出 metaRunQueue 队列弹出 Procedure
        ...
        if (pollResult == null) {
            // 2 其次从 serverRunQueue 队列弹出 Procedure
            pollResult = doPoll(serverRunQueue);
        }
        if (pollResult == null) {
            // 3 最后从 tableRunQueue 队列弹出 Procedure
