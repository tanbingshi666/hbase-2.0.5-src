...
public class CreateTableProcedure
        ...
        try {
            switch (state) {
                case CREATE_TABLE_PRE_OPERATION:
                    ...
                    // 协处理器操作
                    preCreate(env);
                    // 设置下一个状态为 CREATE_TABLE_WRITE_FS_LAYOUT
                ...
                case CREATE_TABLE_WRITE_FS_LAYOUT:
                    ...
                    // 创建表在 HDFS 的文件布局以及创建 HRegion (可能多个 反射创建)
                    /**
                     * 一个 HRegion 对象
                     * 1 有 N 多个 HStore (根据列簇个数)
                     * 2 一个 HStore 存在一个 DefaultMemStore + N 多个 HStoreFile
                     * 3 DefaultMemStore 内部封装了一个 MutableSegment
                     * 4 MutableSegment 封装了 CellSet
                     * 5 CellSet 封装了内存跳表数据结构 ConcurrentSkipListMap
                     */
                    newRegions = createFsLayout(env, tableDescriptor, newRegions);
                    // 设置下一个状态为 CREATE_TABLE_ADD_TO_META
                ...
                case CREATE_TABLE_UPDATE_DESC_CACHE:
                    ...
                    // 设置下一个状态为 CREATE_TABLE_POST_OPERATION
    ...
    protected static List<RegionInfo> createFsLayout(final MasterProcedureEnv env,
        ...
        // 获取创建表在 HDFS 的临时目录
        // 格式为 /hbase/data/{namespace}/{tableName}
