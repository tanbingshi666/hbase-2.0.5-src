...
class AsyncRequestFutureImpl<CResult> implements AsyncRequestFuture {
    ...
    void groupAndSendMultiAction(List<Action> currentActions, int numAttempt) {
        // 计算哪些 Action 需要发送到哪些 RegionServer (分组操作)
        Map<ServerName, MultiAction> actionsByServer = new HashMap<>();
        ...
        for (Action action : currentActions) {
            // 寻找 Action(Put) 对应的 Region 所在集群 RegionServer 节点信息
            ...
            if (loc == null || loc.getServerName() == null) {
                ...
                // 缓存 Put 对应的 Region 名称
                byte[] regionName = loc.getRegionInfo().getRegionName();
                // 分组发送 Action 到具体的 RegionServer
                /**
                 * 分组逻辑：
                 * 1 哪些 Action 发送到哪些 RegionServer
                 * 2 哪些 Action 属于哪个 Region
                 * 对应关系
                 * ServerName(RegionSer) -> MultiAction
                 * MultiAction -> Map(RegionName -> N 多个 Action)
                 */
    ...
    private RegionLocations findAllLocationsOrFail(Action action, boolean useCache) {
        ...
        try {
            // 找到 Put RowKey 对应 region 在哪一台 RegionServer 信息
            loc = asyncProcess.connection.locateRegion(
                    // 表名
                    tableName,
                    // Put 对应 RowKey
                    action.getAction().getRow(),
                    useCache,
    ...
    void sendMultiAction(Map<ServerName, MultiAction> actionsByServer,
        ...
        // 获取批量提交 Put 对应哪些 RegionServer 个数
        ...
        for (Map.Entry<ServerName, MultiAction> e : actionsByServer.entrySet()) {
            ...
            for (Runnable runnable : runnables) {
                if ((--actionsRemaining == 0) && reuseThread
                    ...
                    try {
                        // 提交 Actions 调用 SingleServerRequestRunnable.run()
    ...
    private Collection<? extends Runnable> getNewMultiActionRunnable(ServerName server,
        ...
        for (DelayingRunner runner : actions.values()) {
            ...
            // 创建 SingleServerRequestRunnable
            Runnable runnable = createSingleServerRequest(
                    runner.getActions(),
                    numAttempt,
                    server,
                    callsInProgress);
            ...
            // 包装 SingleServerRequestRunnable
            runnable = TraceUtil.wrap(runnable, traceText);
            // 添加 SingleServerRequestRunnable
