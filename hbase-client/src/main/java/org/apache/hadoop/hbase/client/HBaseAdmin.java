...
public class HBaseAdmin implements Admin {
    ...
    public void createTable(final TableDescriptor desc, byte[][] splitKeys)
        ...
        get(
                // 异步创建表
                createTableAsync(desc, splitKeys), syncWaitTimeout, TimeUnit.MILLISECONDS
        );
    }
    ...
    public Future<Void> createTableAsync(final TableDescriptor desc, final byte[][] splitKeys)
        ...
        // splitKeys 二维数据 (startRowKey, endRowKey)
        if (splitKeys != null && splitKeys.length > 0) {
            // 排序 (1,4,3,4,5) -> (1,3,4,4,5)
            Arrays.sort(splitKeys, Bytes.BYTES_COMPARATOR);
            // Verify there are no duplicate split keys
            // 去重 (1,3,4,4,5) -> (1,3,4,5)
            byte[] lastKey = null;
        ...
        // 执行创建表 (发送 RPC 请求给 HMaster 的 MasterRpcServices.createTable())
        CreateTableResponse response = executeCallable(
                new MasterCallable<CreateTableResponse>(
                    ...
                    protected CreateTableResponse rpcCall() throws Exception {
                        ...
                        // 调用 HMaster 的 RPC 服务 MasterRpcServices.createTable()
