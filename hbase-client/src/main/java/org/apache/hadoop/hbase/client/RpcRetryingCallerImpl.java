...
public class RpcRetryingCallerImpl<T> implements RpcRetryingCaller<T> {
    ...
    public T callWithRetries(RetryingCallable<T> callable, int callTimeout)
        ...
        for (int tries = 0; ; tries++) {
            ...
            try {
                ...
                // 对于创建 hbase 表而言 调用 MasterCallable.prepare()
                callable.prepare(tries != 0);
                interceptor.intercept(context.prepare(callable, tries));
                // 最终调用 MasterCallable.rpcCall()
    ...
    public T callWithoutRetries(RetryingCallable<T> callable, int callTimeout)
        ...
        try {
            // 调用 ScannerCallableWithReplicas.prepare()
            // 针对 Scan 请求调用 ScannerCallable.prepare()
            callable.prepare(false);
            // 调用 ScannerCallableWithReplicas.call()
            // 针对 Scan 请求调用 ScannerCallable.call()
