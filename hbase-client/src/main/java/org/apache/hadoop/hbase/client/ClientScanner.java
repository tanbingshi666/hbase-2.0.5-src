...
public abstract class ClientScanner extends AbstractClientScanner {
    ...
    public ClientScanner(final Configuration conf, final Scan scan, final TableName tableName,
        ...
        // 创建扫描 meta 表数据缓存 默认 CompleteScanResultCache
        this.scanResultCache = createScanResultCache(scan);
        // 初始化缓存 也即 cache = new ArrayDeque<>()
        initCache();
    }
    ...
    protected boolean moveToNextRegion() {
        ...
        // 创建 ScannerCallableWithReplicas
        this.callable =
    ...
    private Result[] call(ScannerCallableWithReplicas callable, RpcRetryingCaller<Result[]> caller,
        ...
        // 调用 RpcRetryingCallerImpl.callWithoutRetries()
        // callable = ScannerCallableWithReplicas
    ...
    protected Result nextWithSyncCache() throws IOException {
        ...
        // 如果第一次不知道 meta 表在哪个 RegionServer 则需要连接 ZK 获取对应的信息
        // 加载缓存 也即扫描 meta 表 (发送 RPC 请求)
        loadCache();
    ...
    protected void loadCache() throws IOException {
        ...
        for (; ; ) {
            ...
            try {
                ...
                // is already non-null.
                // 如果第一次不知道 meta 表在哪个 RegionServer 则需要连接 ZK 获取对应的信息 最终调用 ConnectionImplementation.locateRegion()
                // 扫描 meta 表
            ...
            // 扫描 meta 表得到的结果缓存在 scanResultCache
