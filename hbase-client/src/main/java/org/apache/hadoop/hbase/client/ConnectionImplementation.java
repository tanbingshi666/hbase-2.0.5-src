...
class ConnectionImplementation implements ClusterConnection, Closeable {
    ...
    ConnectionImplementation(Configuration conf,
        ...
        // 加载 hbase-default.xml hbase-site.xml 配置文件
        ...
        // 创建 ServerStatisticTracker
        this.stats = ServerStatisticTracker.create(conf);
        // 拦截器
        this.interceptor = (new RetryingCallerInterceptorFactory(conf)).build();
        // 创建 RpcControllerFactory
        this.rpcControllerFactory = RpcControllerFactory.instantiate(conf);
        // 创建 RpcRetryingCallerFactory
        this.rpcCallerFactory = RpcRetryingCallerFactory.instantiate(conf, interceptor, this.stats);
        // 创建 NoBackoffPolicy
        this.backoffPolicy = ClientBackoffPolicyFactory.create(conf);

        // 创建异步批量处理器 AsyncProcess
        ...
        try {
            // 创建连接 ZK 对象 ZKAsyncRegistry
            this.registry = AsyncRegistryFactory.getRegistry(conf);
            // 从 ZK 查询 hbase 集群 ID
            retrieveClusterId();

            // 创建 hbase 集群客户端 NettyRpcClient
    ...
    public RegionLocations locateRegion(final TableName tableName, final byte[] row, boolean useCache,
        ...
        // 如果是 meta 表 也即连接 ZK 获取 meta 的位置信息并缓存到 MetaCache
        if (tableName.equals(TableName.META_TABLE_NAME)) {
            return locateMeta(tableName, useCache, replicaId);
        } else {
            // Region not in the cache - have to go to the meta RS
            // 普通用户表 如果 Put 对应的 region 信息不在缓存
            // 则先找到 meta 在哪个 RegionServer 然后根据 RowKey scan meta 表
            // 找到 Put 应该存放在哪个RegionServer 的哪个 region
    ...
    private RegionLocations locateMeta(final TableName tableName,
        ...
        synchronized (metaRegionLock) {
            ...
            // 连接 ZK 获取 meta 表 region 位置
            locations = get(this.registry.getMetaRegionLocation());
            if (locations != null) {
                // 更新 meta 表对应的 region 位置信息
    ...
    private RegionLocations locateRegionInMeta(TableName tableName, byte[] row, boolean useCache,
        ...
        for (int tries = 0; ; tries++) {
            ...
            try {
                ...
                // 创建 ReversedClientScanner 用户扫描 meta 表
                try (ReversedClientScanner rcs = new ReversedClientScanner(
                        conf, s,
                        // hbase 的元数据 meta 表
                    ...
                    for (; ; ) {
                        /**
                         * 如果第一次进来 还没有连接 ZK 获取 meta 表在哪个 RegionServer
                         * 则 next() 则需要连接 ZK 最终调用 ConnectionImplementation.locateRegion()
                         */
                        // 扫描 meta 表获取元数据
                        ...
                        // not contains us.
                        // 如果扫描结果 RegionInfo RowKey 范围不包含用户表的 rowKey 则报错
                        if (!regionInfo.containsRow(row)) {
                            throw new IOException(
                                    "Unable to find region for " + Bytes.toStringBinary(row) + " in " + tableName);
                        }
                        // 获取扫描 RegionInfo 的 RegionServer 信息
    ...
    private final class MasterServiceStubMaker {
        ...
        private MasterProtos.MasterService.BlockingInterface makeStubNoRetries()
            ...
            // 查询 ZK /hbase/master 节点数据获取 hbase HMaster RPC 服务信息
            ...
            // 获取 HBase RPC 服务 MasterRpcServices 的 RPC 客户端
            MasterProtos.MasterService.BlockingInterface stub =
            ...
            // 发送 RPC 请求给 MasterRpcServices RPC 服务判断是否连接成功
        ...
        MasterProtos.MasterService.BlockingInterface makeStub() throws IOException {
            ...
            synchronized (masterLock) {
                ...
                if (!closed) {
                    try {
                        // 查询 ZK 获取 HMaster RPC 地址并连接返回 RPC 客户端
